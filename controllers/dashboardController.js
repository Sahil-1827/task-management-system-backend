...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const currentPeriodQuery = { ...taskQuery, createdAt: { $gte: thirtyDaysAgo } };
        const lastPeriodQuery = { ...taskQuery, createdAt: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } };
        ...
        // 2. Pending Tasks
        const pendingQuery = { ...taskQuery, status: { $in: ['To Do', 'In Progress'] } };
        ...
        const completedCurrentPeriod = await Task.countDocuments({ ...completedQuery, updatedAt: { $gte: thirtyDaysAgo } });
        const completedLastPeriod = await Task.countDocuments({ ...completedQuery, updatedAt: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } });
        const completedTrend = getTrend(completedCurrentPeriod, completedLastPeriod);
        ...
        const teamsCurrentPeriod = await Team.countDocuments({ ...teamQuery, createdAt: { $gte: thirtyDaysAgo } });
        const teamsLastPeriod = await Team.countDocuments({ ...teamQuery, createdAt: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } });
        const teamsTrend = getTrend(teamsCurrentPeriod, teamsLastPeriod);

        // 5. High Priority Tasks
        const highPriorityTasks = await Task.countDocuments({ ...taskQuery, priority: 'High' });
